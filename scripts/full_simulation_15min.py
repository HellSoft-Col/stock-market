#!/usr/bin/env python3
"""
Full 15-Minute Trading Simulation - 12 Concurrent Teams
======================================================

Complete trading simulation with:
- 12 teams running concurrently in separate threads
- Diverse trading strategies per team
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'full_simulation_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class AdvancedTradingClient:
    ...
    async def wait_for_response(self, expected_types: List[str], order_id: str | None = None, timeout: float = 3.0) -> Dict:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if msg_type in expected_types:
                    if order_id:
                        if message.get("clOrdID") == order_id:
                            return message
                    else:
                        return message
                elif msg_type == "ERROR":
                    return message
                elif msg_type in ["TICKER", "INVENTORY_UPDATE", "OFFER"]:
    ...
    async def send_order(self, side: str, product: str, quantity: int, limit_price: float | None = None, mode: str = "MARKET") -> bool:
        ...
        if self.websocket:
            ...
            # Don't wait for response in fast trading to avoid blocking
            return True
    ...
    async def send_production(self, product: str, quantity: int) -> bool:
        ...
        if self.websocket:
            ...
            response = await self.wait_for_response(["INVENTORY_UPDATE"], timeout=2.0)
            if response.get("type") == "INVENTORY_UPDATE":
                self.stats['productions_successful'] += 1
                return True
    ...
    async def _execute_trading(self):
        ...
        if self.config.strategy == "Aggressive":
            await self._aggressive_trading()
        elif self.config.strategy == "Conservative":
            await self._conservative_trading()
        elif self.config.strategy == "Market_Maker":
            await self._market_making()
        elif self.config.strategy == "Arbitrage":
            await self._arbitrage_trading()
        else:
            await self._random_trading()
    
    async def _aggressive_trading(self):
        """Aggressive trading: frequent market orders"""
    ...
    async def _conservative_trading(self):
        """Conservative trading: limit orders with better prices"""
        product = random.choice(self.config.preferred_products)
        
        if product in self.market_data:
            market = self.market_data[product]
            
            if random.choice([True, False]):  # Buy
                if market['ask'] is not None:
                    # Place bid below current ask
                    price = market['ask'] * 0.95
                    await self.send_order("BUY", product, 1, price, "LIMIT")
                    self.logger.debug(f"🛡️ Conservative BUY: 1 {product} @ ${price:.2f}")
            else:  # Sell
                if market['bid'] is not None:
                    # Place ask above current bid
                    price = market['bid'] * 1.05
                    await self.send_order("SELL", product, 1, price, "LIMIT")
                    self.logger.debug(f"🛡️ Conservative SELL: 1 {product} @ ${price:.2f}")
    
    async def _market_making(self):
        ...
        if product in self.market_data:
            ...
            if market['mid'] is not None:
                spread = 0.10  # 10 cent spread
                buy_price = market['mid'] - spread
                sell_price = market['mid'] + spread
                
                # Place both orders
                await self.send_order("BUY", product, 1, buy_price, "LIMIT")
                await self.send_order("SELL", product, 1, sell_price, "LIMIT")
                self.logger.debug(f"📊 Market making {product}: BUY @ ${buy_price:.2f}, SELL @ ${sell_price:.2f}")
    
    async def _arbitrage_trading(self):
        """Simple arbitrage: look for price differences"""
        # This is a simplified version - in reality would be more complex
        if len(self.market_data) >= 2:
            products = list(self.market_data.keys())
            random.shuffle(products)
            
            # Simple strategy: buy low-priced, sell high-priced
            for product in products[:2]:
                market = self.market_data[product]
                if market['mid'] is not None and market['mid'] < 15:  # Buy "cheap" products
                    await self.send_order("BUY", product, 1, mode="MARKET")
                    self.logger.debug(f"🔄 Arbitrage BUY: {product} @ ${market['mid']:.2f}")
                    break
    
    async def _random_trading(self):
        """Random trading for testing"""
        product = random.choice(self.config.preferred_products)
        side = random.choice(["BUY", "SELL"])
        quantity = random.randint(1, 2)
        
        if random.choice([True, False]):  # Market order
            await self.send_order(side, product, quantity, mode="MARKET")
        else:  # Limit order
...
class SimulationManager:
    """Manages the entire 15-minute simulation with all teams"""
    ...
    def setup_teams(self):
        ...
        team_configs = [
            TeamConfig(
                ...
                production_frequency=15,  # Every 15 seconds
                trading_frequency=25     # Every 25 seconds
            ),
            TeamConfig(
                ...
                trading_frequency=10     # Very frequent trading
            ),
            TeamConfig(
                ...
                trading_frequency=45     # Slow, careful trading
            ),
            TeamConfig(
            ...
            TeamConfig(
                ...
                trading_frequency=15     # Fast arbitrage
            ),
            TeamConfig(
                ...
                production_frequency=40,  # Less production
                trading_frequency=12     # More trading
            ),
            TeamConfig(
                ...
                production_frequency=12,  # Very productive
            ...
            TeamConfig(
                ...
                trading_frequency=8      # Very aggressive
            ),
            TeamConfig(
                ...
                trading_frequency=50     # Very conservative
            ),
            TeamConfig(
    ...
    async def run_simulation(self, duration_minutes: int = 15):
        ...
        logger.info("🚀 STARTING 15-MINUTE FULL TRADING SIMULATION")
        logger.info("="*60)
        ...
        # Monitor progress
        progress_task = asyncio.create_task(self._monitor_progress(connected_teams, duration_minutes))
    ...
    async def _monitor_progress(self, teams: List[AdvancedTradingClient], duration_minutes: int):
        ...
        while time.time() < end_time:
            ...
            if current_time - last_report >= report_interval:
                ...
                logger.info(f"\n📊 PROGRESS REPORT - {elapsed:.1f}/{duration_minutes} minutes")
                logger.info("-" * 50)
                
                total_orders = sum(team.stats['orders_sent'] for team in teams)
                total_fills = sum(team.stats['fills_received'] for team in teams)
                total_productions = sum(team.stats['productions_sent'] for team in teams)
                ...
                logger.info(f"⏰ Time Remaining: {remaining:.1f} minutes")
                
                # Show top 3 most active teams
                sorted_teams = sorted(teams, key=lambda t: t.stats['orders_sent'], reverse=True)
                logger.info("🏆 Most Active Teams:")
                for i, team in enumerate(sorted_teams[:3]):
                    logger.info(f"  {i+1}. {team.config.name}: {team.stats['orders_sent']} orders, {team.stats['fills_received']} fills")
...
async def main():
    """Main entry point for 15-minute simulation"""
    manager = SimulationManager()
    manager.setup_teams()
    
    try:
        await manager.run_simulation(duration_minutes=15)
