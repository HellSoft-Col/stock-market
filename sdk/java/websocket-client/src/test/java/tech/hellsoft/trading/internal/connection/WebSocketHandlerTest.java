...
class WebSocketHandlerTest {
  ...
  void shouldRejectNullOnMessage() {
    assertThrows(
        IllegalArgumentException.class, () -> new WebSocketHandler(null, onError, onClose));
  }

  @Test
  void shouldRejectNullOnError() {
    assertThrows(
        IllegalArgumentException.class, () -> new WebSocketHandler(onMessage, null, onClose));
  }

  @Test
  void shouldRejectNullOnClose() {
    assertThrows(
        IllegalArgumentException.class, () -> new WebSocketHandler(onMessage, onError, null));
  }
  ...
  void shouldHandleExceptionInMessageCallback() {
    doThrow(new RuntimeException("Callback error")).when(onMessage).accept(anyString());

    handler = new WebSocketHandler(onMessage, onError, onClose);

    assertDoesNotThrow(() -> handler.onText(mockWebSocket, "{\"type\":\"TEST\"}", true));
  ...
  void shouldHandleExceptionInCloseCallback() {
    doThrow(new RuntimeException("Close callback error")).when(onClose).run();

    handler = new WebSocketHandler(onMessage, onError, onClose);

    assertDoesNotThrow(() -> handler.onClose(mockWebSocket, 1000, "Normal closure"));
  ...
  void shouldHandleExceptionInErrorCallback() {
    doThrow(new RuntimeException("Error callback error")).when(onError).accept(any());

    handler = new WebSocketHandler(onMessage, onError, onClose);
    Throwable testError = new RuntimeException("Test error");

    assertDoesNotThrow(() -> handler.onError(mockWebSocket, testError));
  ...
  void shouldNotInterfereBetweenMessages() {
    ...
    Consumer<String> trackingCallback =
        msg -> {
  ...
  void shouldHandleConcurrentCallbacks() throws InterruptedException {
    ...
    Consumer<String> concurrentCallback =
        msg -> {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Thread.startVirtualThread(() -> handler.onText(mockWebSocket, "message" + messageNum, true));
    }
