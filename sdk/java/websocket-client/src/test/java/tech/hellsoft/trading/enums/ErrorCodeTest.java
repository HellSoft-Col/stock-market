...
class ErrorCodeTest {
  ...
  static Stream<Arguments> errorCodeValues() {
    return Stream.of(
        ...
        Arguments.of(
            ErrorCode.UNAUTHORIZED_PRODUCTION, "UNAUTHORIZED_PRODUCTION", ErrorCode.Severity.ERROR),
        Arguments.of(ErrorCode.OFFER_EXPIRED, "OFFER_EXPIRED", ErrorCode.Severity.INFO),
        Arguments.of(
            ErrorCode.RATE_LIMIT_EXCEEDED, "RATE_LIMIT_EXCEEDED", ErrorCode.Severity.WARNING),
        Arguments.of(
            ErrorCode.SERVICE_UNAVAILABLE, "SERVICE_UNAVAILABLE", ErrorCode.Severity.TRANSIENT),
        Arguments.of(
            ErrorCode.INSUFFICIENT_INVENTORY, "INSUFFICIENT_INVENTORY", ErrorCode.Severity.ERROR),
  ...
  void shouldReturnCorrectValue(
      ErrorCode code, String expectedValue, ErrorCode.Severity expectedSeverity) {
  ...
  void shouldDeserializeFromJson(
      ErrorCode expectedCode, String jsonValue, ErrorCode.Severity severity) {
  ...
  void shouldHaveAllSeveritiesRepresented() {
    assertTrue(
        Stream.of(ErrorCode.values()).anyMatch(c -> c.getSeverity() == ErrorCode.Severity.FATAL));
    assertTrue(
        Stream.of(ErrorCode.values()).anyMatch(c -> c.getSeverity() == ErrorCode.Severity.ERROR));
    assertTrue(
        Stream.of(ErrorCode.values()).anyMatch(c -> c.getSeverity() == ErrorCode.Severity.WARNING));
    assertTrue(
        Stream.of(ErrorCode.values()).anyMatch(c -> c.getSeverity() == ErrorCode.Severity.INFO));
    assertTrue(
        Stream.of(ErrorCode.values())
            .anyMatch(c -> c.getSeverity() == ErrorCode.Severity.TRANSIENT));
  }
}
