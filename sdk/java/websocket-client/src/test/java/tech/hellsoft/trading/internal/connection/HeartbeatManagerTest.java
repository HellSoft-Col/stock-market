...
class HeartbeatManagerTest {
  ...
  void shouldStartHeartbeatManager() {
    manager =
        new HeartbeatManager(Duration.ofMillis(100), Duration.ofSeconds(5), sendPing, onTimeout);
  ...
  void shouldResetTimeoutWhenPongReceived() throws InterruptedException {
    manager =
        new HeartbeatManager(Duration.ofMillis(50), Duration.ofMillis(200), sendPing, onTimeout);
  ...
  void shouldStopSendingPingsAfterStop() throws InterruptedException {
    manager =
        new HeartbeatManager(Duration.ofMillis(50), Duration.ofSeconds(5), sendPing, onTimeout);
  ...
  static Stream<Duration> invalidDurations() {
    return Stream.of(null, Duration.ZERO, Duration.ofSeconds(-1), Duration.ofMillis(-100));
  }
  ...
  void shouldHandleExceptionInSendPing() throws InterruptedException {
    ...
    Consumer<PingMessage> faultySender =
        msg -> {
    ...
    manager =
        new HeartbeatManager(Duration.ofMillis(50), Duration.ofSeconds(5), faultySender, onTimeout);
  ...
  void shouldHandleExceptionInTimeoutCallback() throws InterruptedException {
    ...
    manager =
        new HeartbeatManager(
            Duration.ofMillis(50), Duration.ofMillis(150), sendPing, faultyTimeout);
