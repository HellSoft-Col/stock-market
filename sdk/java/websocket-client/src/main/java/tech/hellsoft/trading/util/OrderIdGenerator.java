 ...
 *
 * <p>This class provides atomic counter-based order ID generation that is safe for use
 * across multiple threads and concurrent trading strategies. Using this generator prevents
 * duplicate order ID errors that can occur when multiple orders are placed simultaneously.
 *
 ...
 * <ul>
 *   <li>{@code PREFIX}: Custom identifier (e.g., team name, strategy name)</li>
 *   <li>{@code TIMESTAMP}: Milliseconds since epoch (for sorting/debugging)</li>
 *   <li>{@code COUNTER}: Atomic counter (ensures uniqueness within same millisecond)</li>
 * </ul>
 *
 * <p>Example generated IDs:
 * <ul>
 *   <li>{@code TEAM-A-1732475123456-1}</li>
 *   <li>{@code TEAM-A-1732475123456-2}</li>
 *   <li>{@code STRAT-MM-1732475123457-1}</li>
 * </ul>
 *
...
public class OrderIdGenerator {
     ...
     *
     * <p>This method is equivalent to {@link #next()} and provided for consistency
     * with {@link AtomicLong#incrementAndGet()} naming conventions.
     *
     * @return a unique order ID string
     * @see #next()
     */
     ...
     *
     * <p>Note: This method returns the same result as {@link #incrementAndGet()}.
     * The counter is incremented before ID generation, so both methods are equivalent.
     *
     * @return a unique order ID string
     * @see #next()
     */
     ...
     *
     * <p><b>Warning:</b> This method should rarely be used in production code as it may
     * lead to duplicate IDs if called while orders are being placed. It's primarily
